...
const areas = {
  list: [
    { pathPoints: [2, 3, 4], color: '#A5A5A5', mouseReplace: false },
    { pathPoints: [2, 1, 6], color: '#EAEAEA', mouseReplace: false },
  ],
}

const areas2 = {
  list: [
    { pathPoints: [2, 3, 4], color: '#231B3C', mouseReplace: false },
    { pathPoints: [2, 1, 6], color: '#231B2C', mouseReplace: false },
    { pathPoints: [4, 5, 6], color: '#231B4C', mouseReplace: false },
    { pathPoints: [2, 4, 6], color: '#1A192E', mouseReplace: false },
  ],
}

const lines5 = {
  list: [
    { points: { a: 2, b: 4 }, color: '#4A5793', mouseJointPoint: null },
    { points: { a: 4, b: 6 }, color: '#53436D', mouseJointPoint: null },
    { points: { a: 6, b: 2 }, color: '#897590', mouseJointPoint: null },
  ],
}

const lines6 = {
  list: [
    { points: { a: 0, b: 3 }, color: '#4A5793', mouseJointPoint: 'b' },
    { points: { a: 0, b: 5 }, color: '#53436D', mouseJointPoint: 'b' },
    { points: { a: 0, b: 1 }, color: '#897590', mouseJointPoint: 'b' },
    { points: { a: 2, b: 4 }, color: '#4A5793', mouseJointPoint: null },
    { points: { a: 4, b: 6 }, color: '#53436D', mouseJointPoint: null },
    { points: { a: 6, b: 2 }, color: '#897590', mouseJointPoint: null },
  ],
...
function fillCanvas() {
  ...
  ctx.canvas.style.backgroundColor = palleteConfig.backgroundColor
  ...
  //     ctx: ctx,
  //     color: palleteConfig.color,
  //     backgroundColor: 'transparent',
  //     areas,
  //   })
  // )

  this.polygonPattern = new PolygonPattern(
    configFunc({
      width: ctx.canvas.width,
      height: ctx.canvas.height,
      ctx: ctx,
  ...
  this.polygonPattern4 = new PolygonPattern(
    configFunc({
      width: ctx.canvas.width,
      height: ctx.canvas.height,
      ctx: ctx,
  ...
  this.polygonPattern5 = new PolygonPattern(
    configFunc({
      width: ctx.canvas.width,
      height: ctx.canvas.height,
      ctx: ctx,
  ...
  this.polygonPattern6 = new PolygonPattern(
    configFunc({
      width: ctx.canvas.width,
      height: ctx.canvas.height,
      ctx: ctx,
  ...
  const drawForMousePosition = () => {
    this.canvasRef.addEventListener('mousemove', (event) => {
      ...
      this.polygonPattern.redraw(
        event.pageX - rect.left - left,
        event.pageY - rect.top - top,
        dpi
      )
      this.polygonPattern4.redraw(
        event.pageX - rect.left - left,
        event.pageY - rect.top - top,
        dpi
      )
      this.polygonPattern5.redraw(
        event.pageX - rect.left - left,
        event.pageY - rect.top - top,
        dpi
      )
      this.polygonPattern6.redraw(
        event.pageX - rect.left - left,
        event.pageY - rect.top - top,
        dpi
      )
  ...
  const autoDrawForTouchDevices = () => {
    setTimeout(function drawNextPoint() {
      ...
      this.polygonPattern.redraw(nextPoint.x, nextPoint.y, dpi)
      this.polygonPattern4.redraw(nextPoint.x, nextPoint.y, dpi)
      this.polygonPattern5.redraw(nextPoint.x, nextPoint.y, dpi)
      this.polygonPattern6.redraw(nextPoint.x, nextPoint.y, dpi)
